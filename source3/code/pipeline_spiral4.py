...
def pipe_process_first(cnt, repo_path ,commit, hexsha, auth_ext, f_each_commit_file_inf ,f_error, diff_range, snippet_filename):
    error_ctx = ''
...
def pipe_process(cnt, repo_path, commit, hexsha, auth_ext, f_each_commit_file_inf, f_error, diff_range, snippet_filename):
    error_ctx = ''
    command1 = 'git -C ' + repo_path + ' show '
    command2 = 'java -jar ../../package/tokenizer.jar'
...
def excute(repo_path, repo, auth_ext,branch_name,f_cnt, f_each_commit_file_inf, f_error, diff_range, snippet_filename):
    cnt = 0
    ...
    for i, item in enumerate(commits):
        ...
        if not item.parents:
            ...
            cnt, each_commit_cnt = pipe_process_first(cnt, repo_path, commit, target_hexsha, auth_ext, f_each_commit_file_inf, f_error, diff_range, snippet_filename)
        else:
            commit = repo.commit(target_hexsha+'~1')
            cnt , each_commit_cnt = pipe_process(cnt, repo_path, commit, target_hexsha, auth_ext, f_each_commit_file_inf, f_error, diff_range, snippet_filename)
...
def main():
    ...
    with open('../resource/'+cnt_source_filename, 'a', encoding='utf-8') as f_cnt, \
        ...
        excute(repo_path, repo, auth_ext, branch_name, f_cnt, f_each_commit_file_inf, f_error, diff_range, snippet_filename)
    sys.exit(0)
