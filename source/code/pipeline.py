...
def pipe_process(cnt, repo_path, commit, hexsha, auth_ext, f_each_commit_file_inf):
    ...
    command5 = 'diff -u -3 ../resource/before.txt ../resource/after.txt'
    command6 = 'python out_piece_snippet.py'
    command7 = 'python out_snippet_to_txt.py ../resource/snippet.txt'
    ...
    for item in diff:
        ...
        if ch_type == 'M' or ch_type == 'R':
            ...
            if process2.returncode != 0 or process5.returncode != 0:
                ...
                with open('../resource/error_log.txt', 'a', encoding='utf-8') as f_error:
...
def main():
    ...
    branch_name = 'main'
    repo = Repo(repo_path)
    with open('../resource/cnt_sorce_file.txt', 'a', encoding='utf-8') as f_cnt, \
            open('../resource/hexsha_filename.txt', 'a', encoding='utf-8') as f_each_commit_file_inf:
